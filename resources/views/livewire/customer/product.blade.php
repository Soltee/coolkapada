    ...
    <form method="POST" wire:submit.prevent="store">
        ...
        <div class="w-full flex flex-col">
            ...
            @forelse($images as $c)
              ...
              <input 
                class="-mt-4 hidden z-0" 
                type="radio"
                wire:click="attributes({{ $c->id }})"  
                {{ ($loop->first) ? 'checked' : '' }} 
                wire:model.defer="color" value="{{ $c->color }}">
                <span  class="radio_btn mr-2 px-5 py-5  rounded-full  border-2 border-white text-gray-900 cursor-pointer z-10" style="background-color: {{ $c->color }}"
                >
                ...
                <div class=" mt-4 flex flex-row  flex-wrap items-center">
                    ...
                    <div>
                        ...
                        <label  class="custom_radio2 relative flex flex-col">
                            ...
                            <span  
                                class="radio_btn2 mr-2 px-3 py-2 hover:bg-gray-900 hover:text-white rounded  border-2 border-white text-gray-900 cursor-pointer hover:border-gray-400"
                                >
                                {{ $a->size }}
                            </span>
